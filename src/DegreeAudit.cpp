...
void DegreeAudit::findEligibleCourses(const Student& student, const DegreePlan& plan) {
	for (const std::string& courseCode : plan.getRigidCourses()) {
		// Skip courses the student already has a record for — completed
		// courses don't need recommending, and in-progress / planned
		// courses are already on their schedule. Without this guard,
		// the Registration page's "Recommended for You" would suggest
		// courses the student is already enrolled in.
		bool alreadyTaking = false;
		for (const AcademicRecord& rec : student.getRecords()) {
			if (rec.getCourseCode() == courseCode) {
				alreadyTaking = true;
				break;
